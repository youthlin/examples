...
function remove_version_from_style_js( $src ) {
	...
	return str_replace( 'ver=' . get_bloginfo( 'version' ), 'ver=' . $theme_version, $src );
}
...
function lin_enqueue_styles() {
	...
	wp_enqueue_style( 'theme-2021', get_stylesheet_directory_uri() . '/style.css',
...
function lin_wp_footer() {
	...
	wp_enqueue_script( 'theme-2021-js', get_stylesheet_directory_uri() . '/index.js',
...
function lin_ajax_comment() {
	...
	echo $walker->paged_walk( array( $comment ), 1, 1, 1 );
	die();
}
...
add_action( 'wp_footer', 'lin_icp' );
function lin_icp() {
	echo '<a href="https://beian.miit.gov.cn" target="_blank" rel="external nofollow">赣ICP备2021003224号-1</a>';
}